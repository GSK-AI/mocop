...
for FRAC in {1,5,10,25,50,100}; do
        for SPLIT in {1..3}; do
                for SEED in "${!CKPTS[@]}"; do
                        ...
                        echo $SPLIT
                        sbatch  --time=5-00 \
                                --output=${SAVE_DIR}/slurm-chembl20-mocop-%j.out \
                                --mem=40G \
                                --cpus-per-task=4 \
                                --gres=gpu:a6000:1 \
                                ...
                                --wrap "module load anaconda3 && \
                                        ...
                                        python bin/train.py -cn chembl20_mocop.yml \
                                                                ...
                                                                trainer.logger.name=chembl20_mocop_frac_\${FRAC}_split_\${SPLIT}_seed_\${SEED}"
                done
        done
done