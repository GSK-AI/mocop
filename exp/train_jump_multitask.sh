...
for SEED in {0..2}; do
    SPLIT=${SEED}
    sbatch  --time=5-00 \
            --output=${SAVE_DIR}/slurm-jump-multitask-%j.out \
            --mem=500G \
            --cpus-per-task=48 \
            --gres=gpu:a6000:1 \
            --partition=preempted-gpu \
            --export=SPLIT=${SPLIT},SEED=${SEED},SAVE_DIR=${SAVE_DIR},DATA_DIR=${DATA_DIR},CONDA_ENV=${CONDA_ENV} \
            --wrap "module load anaconda3 && \
                    source activate \${CONDA_ENV} && \
                    echo \${PWD} && \
                    which python && \
                    source ./.env && \
                    python bin/train.py -cn jump_multitask.yml \
                                            seed=\${SEED} \
                                            ...
                                            trainer.logger.name=jump_multitask_seed_\${SEED}_split_\${SPLIT}"
done
